...
16. **CODE REVIEW vs GENERATION** — the primary VERB determines the task, not the subject matter:
    - "Review this code", "review this migration", "review this auth handler", "review my PR", "give feedback on X", "give me feedback on", "is this good/safe/idiomatic/correct", "what would you change", "check this code" → **code_review**. The user wants FEEDBACK, not code generation. Even if the subject is a security file or migration — if the verb is "review/give feedback/is this good" → code_review.
    - risk_level for code_review = risk of the SUBJECT: review of migration → risk_level 4; review of auth → risk_level 4; review of styling → risk_level 1.
    - GENERATION verbs ("design", "implement", "build", "create", "write", "fix", "add") → NOT code_review. "Design the migration plan" = database_schema_change/architecture_design.
    - AMBIGUOUS: if prompt says BOTH review AND generation verbs, use the first verb as primary.
