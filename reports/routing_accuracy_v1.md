...
Generated: 2026-06-28T06:58:44.361Z
...
| Metric | Value |
|---|---|
| **Classification accuracy** | **134/139 = 96.4%** |
| **Tier-floor compliance** (no under-route) | **129/139 = 92.8%** |
| Tier-ceiling efficiency (no over-route) | 137/139 = 98.6% |
| Avg design-bar pass rate (visual tasks only) | 94.8% (16 prompts) |
| Total router cost (this run) | $1.5840 |
| Total benchmark cost | $15.8764 |
| Cost reduction | 90.0% |
...
|---|---|---|---|---|---|---|
| api_implementation | 8 | 100% | 100% | 100% | — | anthropic/claude-sonnet-4.6 × 7 |
| architecture_design | 6 | 100% | 100% | 100% | — | google/gemini-2.5-pro × 6 |
| code_review | 5 | 80% | 80% | 100% | — | openai/gpt-4o-mini × 3 |
| creative_generation | 17 | 100% | 100% | 100% | 95% | google/gemini-2.5-pro × 17 |
| database_schema_change | 8 | 88% | 25% | 100% | — | anthropic/claude-sonnet-4.6 × 6 |
| dependency_update | 4 | 100% | 100% | 100% | — | anthropic/claude-sonnet-4.6 × 2 |
| devops_config | 6 | 100% | 100% | 100% | — | google/gemini-2.5-pro × 6 |
| documentation_write | 5 | 100% | 100% | 100% | — | openai/gpt-4o-mini × 5 |
| explanation | 12 | 92% | 100% | 100% | — | openai/gpt-4o-mini × 12 |
| local_bug_fix | 12 | 100% | 92% | 100% | — | anthropic/claude-sonnet-4.6 × 5 |
| multi_file_refactor | 6 | 100% | 100% | 100% | — | anthropic/claude-sonnet-4.6 × 3 |
| performance_optimization | 6 | 100% | 83% | 100% | — | anthropic/claude-sonnet-4.6 × 3 |
| prompt_rewrite_only | 3 | 100% | 100% | 100% | — | openai/gpt-4o-mini × 3 |
| security_sensitive_change | 10 | 100% | 90% | 100% | — | google/gemini-2.5-pro × 9 |
| simple_edit | 11 | 91% | 100% | 100% | — | openai/gpt-4o-mini × 10 |
| test_generation | 11 | 91% | 100% | 82% | — | openai/gpt-4o-mini × 9 |
| ui_change | 9 | 100% | 100% | 100% | — | openai/gpt-4o-mini × 9 |

## 🚨 Failure analysis — what to fix next

### Tier-FLOOR violations (10) — high priority
...
|---|---|---|---|---|
| bug-11 | memory leak in my Node service — heap grows ~50MB per hour | task=local_bug_fix, ≥strong | local_bug_fix → anthropic/claude-sonnet-4.6 (mid) | classifier got "local_bug_fix" right but tier policy allowed mid |
| sec-3 | add CSRF protection to this Express app's mutating endpoints | task=security_sensitive_change, ≥strong | security_sensitive_change → anthropic/claude-sonnet-4.6 (mid) | classifier got "security_sensitive_change" right but tier policy allowed mid |
| db-1 | write a Prisma migration to add NOT NULL 'created_at' to exi | task=database_schema_change, ≥strong | database_schema_change → anthropic/claude-sonnet-4.6 (mid) | classifier got "database_schema_change" right but tier policy allowed mid |
| db-2 | add a unique composite index on (user_id, slug) on posts wit | task=database_schema_change, ≥strong | database_schema_change → anthropic/claude-sonnet-4.6 (mid) | classifier got "database_schema_change" right but tier policy allowed mid |
| db-4 | add foreign key from orders.user_id to users.id without tabl | task=database_schema_change, ≥strong | database_schema_change → anthropic/claude-sonnet-4.6 (mid) | classifier got "database_schema_change" right but tier policy allowed mid |
| db-5 | create a partial unique index on email where deleted_at is n | task=database_schema_change, ≥strong | database_schema_change → anthropic/claude-sonnet-4.6 (mid) | classifier got "database_schema_change" right but tier policy allowed mid |
| db-6 | convert this enum column to a lookup table — preserve histor | task=database_schema_change, ≥strong | database_schema_change → anthropic/claude-sonnet-4.6 (mid) | classifier got "database_schema_change" right but tier policy allowed mid |
| db-7 | add a soft-delete column with a partial index excluding dele | task=database_schema_change, ≥strong | database_schema_change → anthropic/claude-sonnet-4.6 (mid) | classifier got "database_schema_change" right but tier policy allowed mid |
| perf-6 | this loop is hot in flamegraph — vectorize or rewrite for sp | task=performance_optimization, ≥strong | performance_optimization → anthropic/claude-sonnet-4.6 (mid) | classifier got "performance_optimization" right but tier policy allowed mid |
| rev-4 | review this migration for production safety | task=code_review, ≥strong | database_schema_change → anthropic/claude-sonnet-4.6 (mid) | MISCLASSIFIED as "database_schema_change" |

### Classification errors (5)
| Confused | Count |
|---|---|
| simple_edit → multi_file_refactor | 1 |
| database_schema_change → architecture_design | 1 |
| code_review → database_schema_change | 1 |
| test_generation → unknown | 1 |
| explanation → code_review | 1 |

Worst examples:

| ID | Prompt | Expected | Got | Rationale |
|---|---|---|---|---|
| edit-7 | rename file constants.js to config.js and update imports | simple_edit | multi_file_refactor | File rename + update all imports across the project is a cross-file refactor; re |
| db-8 | shard the orders table by tenant_id — design the migration p | database_schema_change | architecture_design | Sharding a production orders table is a critical architectural change affecting  |
| rev-4 | review this migration for production safety | code_review | database_schema_change | Production migration review is critical: schema changes are irreversible, affect |
| edge-3 | make this code testable | test_generation | unknown | Vague refactor request ('make testable') with zero code context requires full re |
| edge-5 | is this code thread-safe | explanation | code_review | User asks for code review/analysis of thread-safety in a small snippet; no code  |

### Design-bar misses (0) — creative/visual tasks below 70%
✅ All visual tasks hit ≥70% of their design bar.
...
- **Actual**: task=`creative_generation`, model=`google/gemini-2.5-pro` (`strong`)
- **Verdict**: ✓task · ✓floor · ✓ceil · design 88%
- **Missing design signals**: themed-bg
...
- **Expected**: task=`creative_generation`, floor=`mid`
- **Actual**: task=`creative_generation`, model=`google/gemini-2.5-pro` (`strong`)
- **Verdict**: ✓task · ✓floor · ✓ceil · design 80%
- **Missing design signals**: score
...
- **Expected**: task=`creative_generation`, floor=`mid`
- **Actual**: task=`creative_generation`, model=`google/gemini-2.5-pro` (`strong`)
- **Verdict**: ✓task · ✓floor · ✓ceil · design 75%
- **Missing design signals**: themed-bg
...
- **Expected**: task=`simple_edit`, floor=`cheap`, ceiling=`mid`
- **Actual**: task=`multi_file_refactor`, model=`anthropic/claude-sonnet-4.6` (`mid`)
- **Verdict**: ✗task · ✓floor · ✓ceil
...
- **Expected**: task=`test_generation`, floor=`cheap`, ceiling=`mid`
- **Actual**: task=`test_generation`, model=`google/gemini-2.5-pro` (`strong`)
- **Verdict**: ✓task · ✓floor · ✗ceil
...
- **Actual**: task=`local_bug_fix`, model=`google/gemini-2.5-pro` (`strong`)
- **Verdict**: ✓task · ✓floor · ✓ceil
...
- **Expected**: task=`local_bug_fix`, floor=`mid`
- **Actual**: task=`local_bug_fix`, model=`anthropic/claude-sonnet-4.6` (`mid`)
- **Verdict**: ✓task · ✓floor · ✓ceil
...
- **Actual**: task=`api_implementation`, model=`anthropic/claude-sonnet-4.6` (`mid`)
- **Verdict**: ✓task · ✓floor · ✓ceil
...
- **Actual**: task=`api_implementation`, model=`openai/gpt-4o` (`strong`)
- **Verdict**: ✓task · ✓floor · ✓ceil
...
- **Actual**: task=`security_sensitive_change`, model=`google/gemini-2.5-pro` (`strong`)
- **Verdict**: ✓task · ✓floor · ✓ceil
...
- **Expected**: task=`security_sensitive_change`, floor=`strong`
- **Actual**: task=`security_sensitive_change`, model=`anthropic/claude-sonnet-4.6` (`mid`)
- **Verdict**: ✓task · ✗FLOOR · ✓ceil
...
- **Actual**: task=`database_schema_change`, model=`anthropic/claude-sonnet-4.6` (`mid`)
- **Verdict**: ✓task · ✗FLOOR · ✓ceil
...
- **Expected**: task=`database_schema_change`, floor=`strong`
- **Actual**: task=`database_schema_change`, model=`google/gemini-2.5-pro` (`strong`)
- **Verdict**: ✓task · ✓floor · ✓ceil
...
- **Expected**: task=`database_schema_change`, floor=`strong`
- **Actual**: task=`architecture_design`, model=`google/gemini-2.5-pro` (`strong`)
- **Verdict**: ✗task · ✓floor · ✓ceil
...
- **Expected**: task=`multi_file_refactor`, floor=`mid`
- **Actual**: task=`multi_file_refactor`, model=`google/gemini-2.5-pro` (`strong`)
- **Verdict**: ✓task · ✓floor · ✓ceil
...
- **Actual**: task=`performance_optimization`, model=`anthropic/claude-sonnet-4.6` (`mid`)
- **Verdict**: ✓task · ✓floor · ✓ceil
...
- **Expected**: task=`performance_optimization`, floor=`mid`
- **Actual**: task=`performance_optimization`, model=`google/gemini-2.5-pro` (`strong`)
- **Verdict**: ✓task · ✓floor · ✓ceil
...
- **Actual**: task=`devops_config`, model=`google/gemini-2.5-pro` (`strong`)
- **Verdict**: ✓task · ✓floor · ✓ceil
...
- **Actual**: task=`dependency_update`, model=`anthropic/claude-sonnet-4.6` (`mid`)
- **Verdict**: ✓task · ✓floor · ✓ceil
...
- **Expected**: task=`dependency_update`, floor=`mid`
- **Actual**: task=`dependency_update`, model=`google/gemini-2.5-pro` (`strong`)
- **Verdict**: ✓task · ✓floor · ✓ceil
...
- **Actual**: task=`code_review`, model=`google/gemini-2.5-pro` (`strong`)
- **Verdict**: ✓task · ✓floor · ✓ceil
...
- **Expected**: task=`test_generation`, floor=`cheap`, ceiling=`mid`
- **Actual**: task=`unknown`, model=`google/gemini-2.5-pro` (`strong`)
- **Verdict**: ✗task · ✓floor · ✗ceil
...
- **Expected**: task=`explanation`, floor=`cheap`, ceiling=`strong`
- **Actual**: task=`code_review`, model=`openai/gpt-4o-mini` (`mid`)
- **Verdict**: ✗task · ✓floor · ✓ceil
