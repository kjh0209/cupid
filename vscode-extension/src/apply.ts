...
export async function applyCodeBlocks(
  ...
  if (blocks.length === 0) return "no code blocks";
  ...
  if (blocks.length > 1 || hasPaths) {
    if (!ws) return applyCodeBlocks(blocks, "newFile");
    let written = 0;
    ...
    for (let i = 0; i < blocks.length; i++) {
      ...
      try {
        ...
        const choice = await vscode.window.showWarningMessage(
          `Cupid: overwrite ${relPath}?`, { modal: true },
          "Overwrite", "Skip",
        );
        if (choice === undefined) break; // user cancelled the modal
